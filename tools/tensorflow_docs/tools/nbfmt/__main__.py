...
def _filter_keys_in_place(data: Dict[str, Any], keep: List[str]) -> None:
...
def clean_root(data: Dict[str, Any], filepath: pathlib.Path) -> None:
  ...
  _filter_keys_in_place(data, keep=["cells", "metadata",
                                    "nbformat_minor", "nbformat"])
  # All metadata is optional according to spec, but we use some of it.
  _filter_keys_in_place(data["metadata"],
                        keep=["accelerator", "colab", "kernelspec"])
  ...
  _filter_keys_in_place(colab, keep=["collapsed_sections", "name", "toc_visible"])
  ...
  _filter_keys_in_place(kernelspec, keep=["display_name", "name"])
...
def clean_cells(data: Dict[str, Any], nb_source: str,
  ...
  for cell in data["cells"]:
    ...
    _filter_keys_in_place(cell_metadata, keep=["id", "cellView"])
